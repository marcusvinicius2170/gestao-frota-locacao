...
export default function TenantPaymentsPage() {
  ...
  const filteredPayments = payments.filter(payment => {
    ...
    if (startDate && endDate) {
      const paymentDate = new Date(payment.weekStart);
      matchesDate = paymentDate >= new Date(startDate) && paymentDate <= new Date(endDate);
    }
  ...
  return (
    <div className="flex min-h-screen bg-[#F7F9FC]">
      ...
      <div className="flex-1 flex flex-col">
        <Header userName="João Silva" userRole="tenant" />
        
        <main className="flex-1 p-8">
          {/* Page Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-[#0A3556] mb-2">Extrato de Pagamentos</h1>
            <p className="text-gray-600">Acompanhe o histórico dos seus pagamentos semanais</p>
          </div>
