...
export default function MaintenancePage() {
  ...
  return (
    <div className="flex min-h-screen bg-[#F7F9FC]">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-[#0A3556] mb-2">Gestão de Manutenções</h1>
                <p className="text-sm sm:text-base text-gray-600">Controle todos os gastos com manutenção da frota</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
              <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200">
                <p className="text-xs sm:text-sm text-gray-600 mb-1">Total de Manutenções</p>
                <p className="text-2xl sm:text-3xl font-bold text-[#0A3556]">{maintenances.length}</p>
              </div>
              <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200">
                <p className="text-xs sm:text-sm text-gray-600 mb-1">Custo Total</p>
                <p className="text-2xl sm:text-3xl font-bold text-red-600">
                  R$ {totalCost.toLocaleString('pt-BR')}
                </p>
              </div>
              <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200">
                <p className="text-xs sm:text-sm text-gray-600 mb-1">Média por Manutenção</p>
            ...
            {/* Cost by Vehicle - Responsivo */}
            <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200 mb-6">
              <h3 className="text-base sm:text-lg font-semibold text-[#0A3556] mb-4">Custo por Veículo</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {Object.values(costByVehicle).map((vehicle) => (
                  <div key={vehicle.plate} className="bg-[#F7F9FC] rounded-lg p-3 sm:p-4">
                    <p className="text-xs sm:text-sm text-gray-600 mb-1">{vehicle.plate}</p>
            ...
            {/* Maintenances Table/Cards - Responsivo */}
              ...
              <div className="hidden lg:block overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-[#F7F9FC]">
                    <tr>
                      <th className="text-left py-3 sm:py-4 px-4 sm:px-6 text-xs sm:text-sm font-semibold text-gray-700">Data</th>
                      <th className="text-left py-3 sm:py-4 px-4 sm:px-6 text-xs sm:text-sm font-semibold text-gray-700">Veículo</th>
                      <th className="text-left py-3 sm:py-4 px-4 sm:px-6 text-xs sm:text-sm font-semibold text-gray-700">Tipo</th>
                      <th className="text-left py-3 sm:py-4 px-4 sm:px-6 text-xs sm:text-sm font-semibold text-gray-700">Descrição</th>
                      <th className="text-left py-3 sm:py-4 px-4 sm:px-6 text-xs sm:text-sm font-semibold text-gray-700">Custo</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredMaintenances.map((maintenance) => (
                      <tr key={maintenance.id} className="border-t border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="py-3 sm:py-4 px-4 sm:px-6">
                          <div>
                            <p className="font-semibold text-gray-900 text-sm">{maintenance.vehiclePlate}</p>
                            <p className="text-xs text-gray-500">{maintenance.vehicleModel}</p>
                          </div>
                        </td>
                        <td className="py-3 sm:py-4 px-4 sm:px-6">
                          <div className="flex items-center gap-2">
                        ...
                        <td className="py-3 sm:py-4 px-4 sm:px-6">
                          <p className="text-xs sm:text-sm text-gray-600">{maintenance.description}</p>
                        </td>
                        <td className="py-3 sm:py-4 px-4 sm:px-6">
                ...
                {filteredMaintenances.map((maintenance) => (
                  <div key={maintenance.id} className="p-4 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="pt-3 border-t border-gray-100 space-y-2">
                      <div className="flex items-center gap-2">
                        <Wrench className="w-4 h-4 text-[#1E88E5]" />
                        <span className="text-sm font-medium text-gray-900">{maintenance.type}</span>
                      </div>
      ...
      {/* Add Maintenance Modal - Responsivo */}
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-xl p-4 sm:p-6 lg:p-8 max-w-2xl w-full my-8">
            <h2 className="text-xl sm:text-2xl font-bold text-[#0A3556] mb-4 sm:mb-6">Registrar Manutenção</h2>
